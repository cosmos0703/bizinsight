  ...
  <header className="absolute top-6 left-1/2 transform -translate-x-1/2 z-50">
    <div className="bg-white/95 backdrop-blur-md shadow-lg rounded-full px-6 py-3 flex items-center gap-8 border border-white/50">
      ...
      <div className="flex items-center gap-2 text-sm">
        {[1, 2, 3].map(s => (
            <div key={s} onClick={() => setStep(s)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full font-semibold cursor-pointer transition-all ${step === s ? 'bg-blue-50 text-blue-700' : 'text-slate-400 hover:text-slate-600'}`}>
                <span className={`w-5 h-5 flex items-center justify-center rounded-full text-xs ${step === s ? 'bg-blue-600 text-white' : 'border border-slate-300'}`}>{s}</span>
                <span>{s === 1 ? '탐색 & 필터' : s === 2 ? '후보 비교' : '상세 분석'}</span>
            </div>
        ))}
      </div>
    </div>
...
const RealMap = ({ data, onMarkerClick, selectedId, heatMapType }) => {
    ...
    const rankedData = useMemo(() => {
        return [...data].sort((a, b) => b.revenue_score - a.revenue_score).map((d, i) => ({ ...d, rank: i + 1 }));
    }, [data]);
    ...
    useEffect(() => {
        const updateMarkers = async () => {
            ...
            const currentIds = new Set(rankedData.map(item => item.id));
            ...
            rankedData.forEach(item => {
                ...
                if (item.rank <= 3) {
                    ...
                    rankBadge = `<div style="position: absolute; top: -12px; left: -10px; background: ${rankColors[item.rank-1]}; color: white; font-size: 10px; font-weight: bold; width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 2px solid white; z-index: 20;">${item.rank}</div>`;
                }
                ...
                content.innerHTML = `
                    ${rankBadge}
                    <div class="px-3 py-1.5 rounded-xl font-bold text-xs shadow-xl flex items-center gap-1 border border-white/20 backdrop-blur-sm text-white" style="background-color: ${isSelected ? '#0f172a' : markerColor}; box-shadow: 0 4px 12px ${markerColor}66; transform: scale(${scale});">
                        <span>${item.name}</span>
                        <span class="opacity-80 border-l border-white/30 pl-1 ml-1">${valText}</span>
                    </div>
                    <div class="w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-t-[8px] mt-[-1px]" style="border-top-color: ${isSelected ? '#0f172a' : markerColor}; transform: scale(${scale});"></div>
                `;
                content.onclick = () => onMarkerClick(item);

                if (markersRef.current[item.id]) {
                    markersRef.current[item.id].content = content;
                    markersRef.current[item.id].zIndex = isSelected ? 200 : zIdx;
                } else {
        ...
        updateMarkers();
    }, [rankedData, selectedId, heatMapType, min, max]);
        ...
        <div className="h-screen w-full bg-slate-50 font-sans text-slate-900 overflow-hidden flex flex-col">
            ...
            <div className="flex-1 p-6 grid grid-cols-12 gap-6 h-full max-h-[calc(100vh-4rem)]">
                ...
                <div className="col-span-12 md:col-span-3 flex flex-col gap-6 h-full overflow-hidden">
                    ...
                    <p className="text-sm text-slate-500 mb-6">행정동 상권을 검색하세요</p>
