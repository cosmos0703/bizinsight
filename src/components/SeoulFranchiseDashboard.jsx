  ...
  <header className="absolute top-6 left-1/2 transform -translate-x-1/2 z-50">
    <div className="bg-white/95 backdrop-blur-md shadow-lg rounded-full px-6 py-3 flex items-center gap-8 border border-white/50">
      ...
      <div className="flex items-center gap-2 text-sm">
        <div onClick={() => setStep(1)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full font-semibold cursor-pointer transition-all ${step === 1 ? 'bg-blue-50 text-blue-700' : 'text-slate-400 hover:text-slate-600'}`}>
        ...
        <div onClick={() => setStep(2)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full font-semibold cursor-pointer transition-all ${step === 2 ? 'bg-blue-50 text-blue-700' : 'text-slate-400 hover:text-slate-600'}`}>
        ...
        <div className="w-8 h-px bg-slate-300"></div>
        <div onClick={() => setStep(3)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full font-semibold cursor-pointer transition-all ${step === 3 ? 'bg-blue-50 text-blue-700' : 'text-slate-400 hover:text-slate-600'}`}>
        ...
        const updateMarkers = async () => {
            ...
            data.forEach(item => {
                ...
                if (heatMapType === 'rent') { val = item.rent_monthly; valText = `${val}만/평`; }
                    ...
                    val = item.revenue; 
                    valText = val >= 10000 ? `${(val/10000).toFixed(1)}억` : `${val}만원`;
                }
...
export default function SeoulFranchiseDashboard() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const mergedData = Object.keys(DONG_COORDINATES).map((dongName, index) => {
                ...
                const rentVal = d.rent; 
                const popVal = d.population;
                const openingsVal = d.openings;
                const revenueVal = d.revenue; 

                const revenueScore = Math.min(100, Math.round((revenueVal / 500000) * 100));
                const compScore = Math.min(100, Math.round((popVal / 50000) * 100));

                return {
                    id: `dong-${index}`, name: dongName, category: '주요 상권',
    ...
    const renderStep1 = () => (
        <div className="relative h-screen w-full bg-slate-50 font-sans text-slate-800 overflow-hidden">
            ...
            <div className="absolute top-24 left-8 bottom-8 w-[340px] flex flex-col gap-4 z-40 pointer-events-none">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                         {cart.map(c => (
                             <div key={c.id} className="bg-slate-50 p-4 rounded-2xl text-left border border-slate-100">
                                 ...
                                 <div className="text-xs text-slate-500 mt-1">예상 매출: {(c.revenue/10000).toFixed(1)}억</div>
                             </div>
                         ))}
                    </div>
